  ...
  async function submitContact(payload){
    ...
    try {
      const res  = await fetch('https://api.emailjs.com/api/v1.0/email/send', {
        ...
        body: JSON.stringify({
          service_id:  'MK-INSIGHT',
          template_id: 'template_wqkmskk',
          user_id:     'IpZuyAIDs-Ctsw9mc',
          template_params: {
...
function spawnParticles(){const c=document.getElementById('successParticles');if(!c)return;c.innerHTML='';const cols=['#ff5252','#ff8a8a','#c40d0d'];for(let i=0;i<18;i++){const p=document.createElement('span');const a=(i/18)*360,d=55+Math.random()*30,s=4+Math.random()*6,col=cols[i%3],dl=Math.random()*0.2,du=0.6+Math.random()*0.4;p.style.cssText=`position:absolute;width:${s}px;height:${s}px;background:${col};border-radius:50%;left:50%;top:50%;transform:translate(-50%,-50%);animation:particle-fly ${du}s ${dl}s ease-out forwards;--dx:${Math.cos(a*Math.PI/180)*d}px;--dy:${Math.sin(a*Math.PI/180)*d}px;opacity:0;`;c.appendChild(p);}}